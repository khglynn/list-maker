...
## Next step (exact)
Begin **Workstream A — hardening** (the scheduler-agnostic durability core), in order:
A1 merge dual show registry → single `show_config.py` (+ divergence test) · A2 `ON CONFLICT` on `insert_mention` · A3 parameterize 90-day filter + `--backfill` flag · A4 per-entity Notion sync state · A5 structured logging · A6 orchestrator-level retry · A7 staleness alert → Slack · A8 tests on extraction + sync · A9 docs (ARCHITECTURE.md + status refresh).
