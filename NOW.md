...
2. **C — Hard Fork — IN PROGRESS (importer bug found 2026-06-07):**
   ...
   - **FIX (next iteration, FULL GATE):** derive the episode `url`/dedup key from the **Taddy episode `uuid`** (always unique) instead of the sometimes-generic `websiteUrl`, in BOTH the upsert (~L285-390) AND `find_existing_episode_id`. **HIGH BLAST RADIUS — core daily import, all 5 shows.** MUST verify existing shows still dedup (their `show_id+title+date` fallback at ~L413-428 should catch them): dry-run `ai-daily-brief` + `sop` imports → expect `imported=0` (zero re-imports) BEFORE committing. Unit-test `find_existing_episode_id`. Codex gate. Then re-import Hard Fork (→199), tiny extraction batch (verify), full `--backfill`. **Surface this core-importer change in the final summary.**
