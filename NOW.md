...
2. **C — Hard Fork — IN PROGRESS (importer bug found 2026-06-07):**
   ...
   - **⏳ FULL backfill RUNNING (background task, ~199 eps, ~$2-3, ~3hr, started ~22:34).** On completion (task-notification) → verify Hard Fork ai_mentions count → sync to Notion.
   - **NEXT — do WHILE the backfill churns (NON-extraction only; do NOT start another OpenAI extraction concurrently — rate limits + load):**
     (a) Create the Hard Fork Notion DB — clone the AI Daily schema (`982dafa0ad374d618e25207e67860e33`) via the `NOTION_TOKEN` integration (Notion API). Integration-share needed (403)? → record + PushNotification Kevin + continue. On success → set `notion_database_id` in show_config + update `tests/test_show_config.py` (`test_only_configured_notion_shows` → {"ai-daily-brief","hard-fork"}).
     (b) B trigger CODE: `.github/workflows/entities.yml` (daily) + the Cloudflare Worker code for trimm + the `run_new_episodes.py` `sys.executable` CI fix.
     (c) D media PROFILE code (the media-extraction prompt/profile — code only, no extraction run yet).
   - **DEFER until the Hard Fork backfill finishes:** sync Hard Fork → Notion; AI Daily catch-up; PCHH/media backfills (all involve OpenAI/heavy runs — keep them sequential).
