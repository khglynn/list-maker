...
2. **C — Hard Fork — IN PROGRESS (importer bug found 2026-06-07):**
   ...
   - **NEXT (this is where C resumes):** tiny extraction batch on Hard Fork (run `extract_entities.py` on ~5 eps → load → verify 5-10 rows look right) → full extraction `--backfill` (~$2-3) → create Hard Fork Notion DB (clone AI Daily `982dafa0ad374d618e25207e67860e33` via NOTION_TOKEN; if it needs an integration share → PushNotification Kevin + continue) → set `notion_database_id` in show_config + update `test_only_configured_notion_shows` → sync → AI Daily catch-up.
