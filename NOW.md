...
- **A6 → NEXT:** orchestrator-level retry — `run_script`/`step_*` failures currently print-and-continue; add bounded retry with backoff for transient subprocess failures + an end-of-run failed-steps summary.
