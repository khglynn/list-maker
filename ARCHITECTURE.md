...
*Live as of 2026-06-11: `GH_PAT` = the reused fine-grained `GITHUB_HG_CLAUDE_TOKEN` (expires 2027-01-20 — dispatch failures alert at the trigger when it does); all `schedule:` blocks removed — the Worker is the only trigger. **Constraint learned the hard way: Workers Free caps cron triggers at 5 PER ACCOUNT** (a 7-cron deploy failed; this file said "per Worker" until 2026-09-03, which is wrong and would mislead anyone budgeting a new Worker's crons — the 2026-06-11 failure couldn't tell the two apart because this was then the account's only cron Worker). The Worker was consolidated to ONE daily cron on 2026-08-26 and worker.js picks the day's dispatches by fire date. Live count on the trimm account, 2026-09-03: two crons total — `list-maker-cron` and `remembrall-hub`. **Second constraint learned the hard way (2026-07-24): Cloudflare cron day-of-week is 1=Sunday..7=Saturday, NOT standard cron's 0=Sunday — the original 1,3,5 values fired Sun/Tue/Thu for six weeks and TAL never auto-synced. Cloudflare Mon=2/Wed=4/Fri=6.***
