...
**Second review (three lenses plus skeptics, 13 confirmed findings, same day):** the hand-off to the music workflow muted every per-show entities check for SOP and TAL, including the songs-still-arriving check built for the July TAL silence — now it covers only the feed check, and only while the music workflow's own feed alert is active. A job cut off by its time limit was silent — every step now has a timeout that fits inside the job's, and a cancelled run is announced. Autocommit had quietly broken the intake's all-or-nothing candidate batch (older posts could be stranded behind the date cursor) — the two batch writes are single transactions again. Flapping is now one problem: "recovered" once it has held two days, a relapse within a week reopens the same issue quietly, reminders count the flips. Also fixed: failed Notion page creates were silent, a narrower manual run could fake a recovery, manual all-shows music runs left issues nothing would close, an unreachable feed silenced its alert's reminders.

**Third review (Codex on round 3, 2026-09-24), and a simplification:** all eight findings reproduced as failing tests first. They clustered where features met — the music hand-off, reopening a flapping check, the time-based recovery hold, shows that couldn't be checked — so instead of more patches the announcer now keeps one invariant per check: its first failure, a weekly word (still failing / failing again / couldn't check), or a verified recovery; nothing else posts. Removed: the entities-to-music hand-off (one show's feed gap can now have two threads, each weekly, and nothing can be silenced), per-show "now also failing" news (Kevin's rule is per check), and the time-based hold. The marker and the close are one edit. Also fixed: a failed Notion create the sync stopped retrying no longer fails forever.

**Fourth review (Codex on the simplified announcer, 2026-09-24):** seven findings reproduced as failing tests, then fixed without adding states: the weekly word now reaches every open issue whatever today's result (including "passing, confirming recovery"), a green streak only extends across consecutive runs of the workflow, an unverified show stays in the failing set, other checks' quiet failures are no longer repeated in a message, a failed Notion create stays failing until a page exists or it has no mentions, and a failed closed-issue lookup keeps the open state. A property test runs 200 random 60-day histories against the rule.
